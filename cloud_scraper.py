...
def crawl_centers():
    ...
    for a in soup.find_all('a', href=True):
        if 'action=inst_view' in a['href'] and 'inst_key=' in a['href']:
            text = a.text.strip()
            if text and text != "詳細":
...
def check_gym_months(center_name, gym_url, state, now):
            ...
            if gym_th:
                ...
                def extract_status(tr):
                    ...
                    for td in tr.find_all('td'):
                        ...
                        if strong and '予約状況' in strong.text:
                            img = td.find('img')
...
def main():
    ...
    for i, c in enumerate(centers):
        ...
        if gym_key:
            ...
            check_gym_months(c['name'], gym_url, state, now)
        else:
